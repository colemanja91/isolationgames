...
RSpec.describe Mutations::PickWinner do
  ...
  def mutation(user_card_id)
    <<~GQL
      mutation {
        pickWinner(gameId: #{game.id}, userCardId: #{user_card_id}) {
          id
          currentRound {
            winner {
              id
  ...
  it "marks the specified card as winner" do
    play_cards
    winner_card = game.current_round.user_cards.first
    IsolationgamesSchema.execute(mutation(winner_card.id), context: { current_user: judge })
    expect(game.reload.current_round.winner).to eq(winner_card)
  end

  it "returns the winning card" do
    play_cards
    winner_card = game.current_round.user_cards.first
    result = IsolationgamesSchema.execute(mutation(winner_card.id), context: { current_user: judge })["data"]["pickWinner"]
    expect(result["currentRound"]["winner"]["id"]).to eq(winner_card.id)
  end
  ...
  it "errors if user is not the judge" do
    ...
    play_cards
    winner_card = game.current_round.user_cards.first
    result = IsolationgamesSchema.execute(mutation(winner_card.id), context: { current_user: user })
